...
interface HighlandStatic {
  ...
  <R>(xs: Promise<R>): Highland.Stream<R>;
   ...
   * });
   */
  pipeline<R>(): Highland.Stream<R>;
  pipeline<R, T>(fn: (arg: Highland.Stream<R>) => Highland.Stream<T>): Highland.Stream<T>;

  zip<T1, T2>(stream1: Highland.Stream<T1>, stream2: Highland.Stream<T2>): Highland.Stream<[T1, T2]>;
  zipAll0<R>(streams: Highland.Stream<R>[]): Highland.Stream<R[]>;
...
declare module Highland {
  ...
  interface Stream<R> extends NodeJS.EventEmitter {
     ...
     * @api public
     */
    reduce1(f: (memo: R, x: R) => R): Stream<R>;
